...
import { LEADS, AGENTS, type LeadRecord } from "../_data/dataset";
...
export function LeadsTable() {
  ...
  const enriched = useMemo(
    () => LEADS.map((l) => ({ lead: l, meta: phoneMeta(l.phoneRaw) })),
    [],
  );
  ...
  const rows = useMemo(() => {
    ...
    return enriched.filter(({ lead, meta }) => {
      if (agent !== "ALL" && lead.agentId !== agent) return false;
  ...
  }, [enriched, q, agent, stage, country]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        />
        <Select value={agent} onChange={setAgent} label="Agent" options={[["ALL", "All agents"] as [string, string], ...AGENTS.map((a) => [a.id, a.name] as [string, string])]} />
        <Select
          value={stage}
          onChange={setStage}
          label="Stage"
          options={[["ALL", "All stages"] as [string, string], ...(Object.keys(STAGE_LABELS) as CanonicalStage[]).map((s) => [s, STAGE_LABELS[s]] as [string, string])]}
        />
        <Select value={country} onChange={setCountry} label="Country" options={[["ALL", "All countries"] as [string, string], ...countries.map((c) => [c, c] as [string, string])]} />
            ...
            {rows.map(({ lead, meta }) => (
              <tr key={lead.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                ...
                <td className="px-3 py-2.5 whitespace-nowrap">
                  ...
                  <span className="text-white/70">{meta.country ?? lead.country ?? "—"}</span>
                </td>
                ...
                <td className="px-3 py-2.5">
                  <Badge variant={(STAGE_VARIANT[lead.stage] ?? "default") as never}>
                    {STAGE_LABELS[lead.stage as CanonicalStage] ?? lead.stage}
                  </Badge>
                  {lead.rawStage?.toLowerCase() === "closing stage" && (
                    <span className="ml-1 text-[10px] text-amber-300/70" title="Normalized from the 'Closing stage' catch-all">
                      ⚑
                    </span>
                  )}
                </td>
  ...
  return (
    <label className="flex items-center gap-1 text-sm">
      ...
      <select
        ...
        className={cn(
          "rounded-md border border-hairline bg-ink-100 px-2.5 py-2 text-sm text-white/80",
          "focus:border-gold/50 focus:outline-none",
        )}
      >
        {options.map(([v, l]) => (
          <option key={v} value={v} className="bg-ink-100">
            {l}
          </option>
        ))}
      </select>
